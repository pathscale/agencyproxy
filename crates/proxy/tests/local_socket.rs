...
use nagoya::reactor::{Addr, Reactor};
use std::{
...
// server's listener, its connections, the clients and the provider processes
// are all registered on it. `nagoya::block_on` polls no reactor, so a threaded
// one is what keeps those sockets moving while the test awaits.

#[test]
fn hello_then_list_runs_uses_the_versioned_local_protocol() {
    ...
    nagoya::block_on(async {
        ...
        let server = ProxyServer::bind(&socket, &handle)
            .await
        ...
        let task = nagoya::spawn(server.serve());
        let client = Client::connect(&socket, &handle)
            .await
...
#[test]
fn idle_daemon_accepts_a_graceful_shutdown() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
        ...
        nagoya::timeout(Duration::from_secs(2), task)
            .await
...
#[test]
fn shutdown_refuses_to_interrupt_an_active_run() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
...
#[test]
fn cancel_acknowledges_only_after_an_uncooperative_provider_is_terminal() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
        ...
        let terminal = nagoya::timeout(Duration::from_secs(2), async {
            loop {
...
#[test]
fn draining_shutdown_rejects_new_runs_and_waits_for_the_active_run() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
...
"#,
        ...
        let late_client = Client::connect(&socket, &handle)
            .await
        ...
        nagoya::timeout(Duration::from_secs(5), task)
            .await
...
#[test]
fn terminating_shutdown_stops_active_runs_before_exit() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
...
fn commands_before_hello_are_rejected() {
    ...
    nagoya::block_on(async {
        ...
        let server = ProxyServer::bind(&socket, &handle)
            ...
            .expect("server should bind");
        let task = nagoya::spawn(server.serve());
        let address = Addr::path(socket.as_os_str().as_bytes()).expect("socket path should fit");
...
#[test]
fn refuses_to_replace_a_regular_file_at_the_socket_path() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
        ...
        let error = ProxyServer::bind(&socket, &handle)
            .await
...
#[test]
fn provider_run_survives_a_client_disconnect_and_replays_on_attach() {
    let reactor = Reactor::start().expect("test reactor should start");
    let handle = reactor.handle();
    nagoya::block_on(async {
        let dir = tempdir().expect("temp dir should exist");
...
"#,
        ...
        let first = Client::connect(&socket, &handle)
            .await
        ...
        let second = Client::connect(&socket, &handle)
            .await
        ...
        let mut latest = 0;
        for _ in 0..8 {
            let frame = nagoya::timeout(Duration::from_secs(5), events.recv())
                .await
