...
//!
//! This stands in for `tokio::sync::broadcast`, which [`Client::subscribe`]
//! returned. nagoya's `sync` has no broadcast, and the shape the callers depend
//! on is small enough to keep here rather than take a crate for: every receiver
//! sees every value sent after it subscribed, a sender never waits, and a
//! receiver that falls more than `capacity` values behind is told how many it
//! missed (`RecvError::Lagged`) and resumes at the oldest value still held.
//!
...
//! observes.
//!
//! The proxy crate carries the same file. The protocol crate is the only one
